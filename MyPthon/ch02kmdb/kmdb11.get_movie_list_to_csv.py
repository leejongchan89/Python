...
# 해당 사이트에서 발급 받은 키
service_key = '31e341bae0ddcec6f78fc13c19f0b59e'


def getDataFromWeb(url):
    # url 정보를 이용하여 해당 웹 사이트에서 json 데이터를 읽어 옵니다.
    req = urllib.request.Request(url) # urllib로 요청 객체를 만든다
    try:
        response = urllib.request.urlopen(req) # 응답 객체
        # http 응답 코드가 성공이면 200을 반환합니다. http 응답코드 ex)404 에러
...
def movieExtractor(pageNumber, pageSize, thisYear): # url 추출 def
    # 영화목록 -> rest방식 -> 기본 요청 url 복사
    ...
    # parameter += '&movieNm=' + '행복의 나라' # 한글 인코딩이 필요 import urllib.parse
    parameter += '&movieNm=' + encoded_movie_name # 한글 인코딩이 필요 import urllib.parse
    ...
    if jsonData == None:
        ...
        try:
            return json.loads(jsonData) #파이썬 객체로 만든다 loads = load string
        except Exception as err:
...
def makeMovieTable(movieData):
    for onemovie in movieData['movieListResult']['movieList']:
        ...
        oneframe = pd.DataFrame(onedict, index=[0]) # 여기서 index=[0]은 1행을 의미
        # print(oneframe)

        global movieTable # 전역 변수임을 알리기 위하여 global 키워드를 사용합니다.

        # 이번에 생성된 데이터 프레임 oneframe을 movieTable에 누적시킵니다.
        movieTable = pd.concat([movieTable, oneframe]) # concat은 결합, oneframe은 하나의 영화 정보를 같는 데이터프레임
    print('-'*30)
...
for thisYear in range(startYear, endYear): # 2024만 해당
    ...
    while True:
        ...
        totalPage = math.ceil(totCnt/pageSize) # ceil 올림
        print('진행 중인 페이지 : ' + str(pageNumber) + '/' + str(totalPage))

        makeMovieTable(movieData)

        if pageNumber == totalPage: #전체 돌리기
        # if pageNumber == 3: # 3페이지까지만
